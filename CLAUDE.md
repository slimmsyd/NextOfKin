...
| Voice | **STT: self-hosted whisper-flow** (faster-whisper `base.en`) on Cloud Run, private, scale-to-zero, behind the `STT_PROVIDER` seam in `src/lib/yourLife/stt.ts` (`/api/your-life/transcribe`, record-then-transcribe). Keeps family audio on our infrastructure (rule #1, conversation-layer only) and ends the recurring Scribe credit cost. ElevenLabs Scribe retained as an OFF-by-default STT fallback. STT Phase 2 = live streaming over the whisper-flow `ws`. **TTS narration is still ElevenLabs** (`/api/voice/[scene]`, `eleven_multilingual_v2`); narration is fixed and tiny so cost is negligible. Self-hosting the voice is a deferred option. See `ADR-002` (amended) + `services/whisper-flow/`. |
