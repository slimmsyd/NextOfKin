...
| Voice | **Self-hosted, default.** STT: **whisper-flow** (faster-whisper `base.en`) on Cloud Run, private, scale-to-zero, behind the `STT_PROVIDER` seam in `src/lib/yourLife/stt.ts` (`/api/your-life/transcribe`, record-then-transcribe). TTS narration: **pre-rendered MisoTTS** clips served static from `public/voice/{scene}.mp3` via the `TTS_PROVIDER` seam (`/api/voice/[scene]`); 5 fixed scenes sourced from `src/lib/voice/scenes.data.json`, rendered offline by `services/miso-tts/`. Both keep family audio on our infrastructure (rule #1, conversation-layer only). ElevenLabs is retained as an OFF-by-default fallback for both. STT Phase 2 = live streaming over the whisper-flow `ws`; TTS Phase B = live spoken replies on Cloud Run GPU. See `ADR-002` (amended), `ADR-003`, `PRE-PUBLIC-LAUNCH-CHECKLIST.md`, and `services/`. **Gate:** confirm the MisoTTS license before relying on it. |
