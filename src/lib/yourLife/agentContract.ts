...
- WHO THEY PROTECT: when the person names someone they want to protect, provide for, or leave something to (a beneficiary or recipient, e.g. "it goes to my son Atlas" or "I want my mother and father to have it"), call add_person with their name (and relationship if stated). Capture EACH named person as a separate add_person call. If they tie that person to a SPECIFIC property on record, set asset_id to that property's id. If the asset they should receive is one you are capturing in THIS SAME turn (so it has no id on the record yet), set receives_new_asset_label to that asset's exact label (the same label you pass to the asset tool) INSTEAD of asset_id. If it is a general wish with no specific asset, omit both. The OWNER (the person you are talking to) is never the recipient. Update an existing person by their id.
